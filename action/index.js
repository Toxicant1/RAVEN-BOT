...
// A helper function to send notifications
const sendNotification = async (client, message) => {
    try {
        if (notificationNumber) {
            ...
            console.log(chalk.yellow("Warning: notificationNumber is not set. Cannot send WhatsApp notification."));
        }
    } catch (error) {
 ...
 client.ev.on('call', async (callData) => {
  ...
  client.ev.on("connection.update", async (update) => {
    ...
    if (connection === "close") {
      ...
      // --- START: MODIFICATIONS FOR DISCONNECT NOTIFICATIONS ---
      const disconnectMessage = `🔴 RAVEN-BOT Disconnected. Reason: ${reason}. Attempting to reconnect...`;
      await sendNotification(client, disconnectMessage);
      // --- END: MODIFICATIONS ---
...
try {
  ...
  console.error("❌ Failed to initialize database:", err.message || err);
      ...
      // --- START: MODIFICATIONS FOR CONNECTED NOTIFICATION ---
      const confirmationMessage = `✅ RAVEN-BOT Connected!\n\n` +
                                  `Session ID loaded successfully. The bot is now online and active.\n\n` +
                                  `Bot Mode: ${mode}\n` +
                                  `Prefix: ${prefix}\n\n` +
                                  `Type your command to continue.`;
      
      await sendNotification(client, confirmationMessage);
      // --- END: MODIFICATIONS ---
    }
  });
